...
class IlluminatorElement {
    ...
    func toString() -> String {
        ...
        return ["\(indent)\(elementDesc) - label: \(label) identifier: \(identifier) value: \(value)", childrenString].joinWithSeparator("\n")
    }
    ...
    private func toXCUIElementHelper(acc: [IlluminatorElement], app: XCUIApplication) -> XCUIElement {
        ...
        let finish = { (top: XCUIElement) in
            return acc.reduce(app) { (parent: XCUIElement, elem) in elem.toXCUIElement(parent: parent) }
        }
    ...
    func toXCUIElement(parent p: XCUIElement) -> XCUIElement {
    ...
    private func toXCUIElementStringHelper(acc: [IlluminatorElement], appString: String) -> String? {
        let finish = { (top: String) -> String? in
            ...
            return acc.reduce(appString) { (parentStr: String, elem) in elem.toXCUIElementString(parentStr) }
            
        }
        
        guard elementType != XCUIElementType.Application else { return finish(appString) }
        guard let parent = parent else {
            print("Warning about toXCUIElementStringHelper not knowing it's done")
    ...
    func toXCUIElementString(parent: String) -> String {
        guard elementType != XCUIElementType.Other || index != nil else { return parent }
        guard !isMainWindow else { return parent }
        let idx: String = index == nil ? "nil" : "\"\(index!)\""
        return parent + ".\(IlluminatorElement.elementStringFromElementType(elementType))[\(idx)]"
    }
    ...
    func childrenMatchingType(elementType: XCUIElementType) -> [String: IlluminatorElement] {
